...
function qumodo_body_classes( $classes ) {
...
add_filter( 'body_class', 'qumodo_body_classes' );

/**
 * Add a pingback url auto-discovery header for single posts, pages, or attachments.
 */

function qumodo_pingback_header() {
	if ( is_singular() && pings_open() ) {
		printf( '<link rel="pingback" href="%s">', esc_url( get_bloginfo( 'pingback_url' ) ) );
	}
}
add_action( 'wp_head', 'qumodo_pingback_header' );

function qumodo_continue_reading_text() {
	$continue_reading = sprintf(
		/* translators: %s: Name of current post. */
		esc_html__( 'Continue reading %s', 'qumodo' ),
...
function qumodo_continue_reading_link_excerpt() {
	global $post;
	return ' ...';
}

// Filter the excerpt more link.
add_filter( 'excerpt_more', 'qumodo_continue_reading_link_excerpt' );
...
function qumodo_print_first_instance_of_block( $block_name, $content = null, $instances = 1 ) {
	$instances_count = 0;
	$blocks_content  = '';
...
if(!function_exists('qumodo_can_show_post_thumbnail')) : 
/**
 ...
 * @return bool
 */
function qumodo_can_show_post_thumbnail() {
	return apply_filters(
		'qumodo_can_show_post_thumbnail',
 ...
 *  
 **/
add_filter( 'get_custom_logo', 'qumodo_custom_logo_class' );


function qumodo_custom_logo_class( $html ) {
...
//Comment Field Order
add_filter( 'comment_form_fields', 'qumodo_comment_fields_custom_order' );
function qumodo_comment_fields_custom_order( $fields ) {
...
if(!function_exists('qumodo_wrapper_start')) {

	function qumodo_wrapper_start( $sidebar = 'right', $column = 8 ) {
...
if(!function_exists('qumodo_wrapper_end')) {

	function qumodo_wrapper_end( $sidebar = 'right' ) {
		?>
		  </div>
		 <?php
			if($sidebar != 'full' && is_active_sidebar( 'blog_sidebar' )){
...
if(!function_exists('qumodo_pagination')){

	function qumodo_pagination(){
...
if ( !function_exists('qumodo_header_footer_builder_id')){

    function qumodo_header_footer_builder_id($posttype, $post_id) {
...
function get_themebuilder_Id( $id, $type = 'header' ) {
 ...
 return qumodo_themebuilder_id($dispaly_list, $id);

}
/*
if(!function_exists('qumodo_header')){
	function qumodo_header( $header_id ) {
	...
	add_action('qumodo_banner_content', 'qumodo_header');
}
*/
if(!function_exists('qumodo_footer')){
	function qumodo_footer( $footer_id ) {
		...
		if ( $footer_id != '' && class_exists( '\Elementor\Plugin' ) ) {
			...
			$footer_text = qumodo_opt('footer_copyright_txt', 'Copyright &copy; 2025 <a href="https://qumodoSoft.com/">qumodo Soft</a> | All rights reserved');
			?>
			<footer id="colophon" class="site-footer text-center">
				<div class="site-info container">
					<?php echo qumodo_kses($footer_text); ?>
				</div><!-- .site-info -->
			</footer><!-- #colophon -->
			<?php
		}
	...
	add_action('qumodo_footer_content', 'qumodo_footer');
}

function get_qumodo_builder_id( $id, $type = 'header' ) {
    ...
    return qumodo_themebuilder_id($display_list, $id);

}

if ( !function_exists('qumodo_header') ) {
	function qumodo_header( $header_id ) {
			$sticky = qumodo_opt('is_sticky', 'show');
	...
	add_action('qumodo_header_content', 'qumodo_header');
 }
...
function get_builder_id($arr, $key) {
 ...
 if ( !function_exists('qumodo_banner_display') ) {
	function qumodo_banner_display( $banner_id, $banner_type ) {
	...
	add_action('qumodo_banner_content', 'qumodo_banner_display', 10, 2);
 }



 
if(!function_exists('qumodo_themebuilder_id')){
function qumodo_themebuilder_id($arr = [], $id='') {
  if(empty($arr)) {
	  return;
  }
  global $post;
